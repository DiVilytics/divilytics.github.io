// ── AUDIO / MEDIA SESSION ──────────────────────────────────────────────────────
// Depends on: games-sheet.js (liveStart, liveTurns, startLive, stopLive, bumpTurn, closeSheet)
...
function _updatePositionState() {
  ...
  if (!liveStart) return;
  const position = (Date.now() - liveStart) / 1000;
...
function _registerMediaHandlers() {
  ...
  navigator.mediaSession.setActionHandler('play', () => {
    ...
    if (liveStart && _bgAudio.paused) _audioPlay();
  });
...
gameEvents.onStop.push(() => {
  _audioPause();
  clearInterval(_metaInterval); _metaInterval = null;
  _updatePositionState();
});

gameEvents.onTurnBump.push(() => {
  _updateMediaMetadata();
});

gameEvents.onClose.push(() => {
  _audioStop();
});
